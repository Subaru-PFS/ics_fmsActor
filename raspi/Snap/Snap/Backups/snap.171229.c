 ...
 * calling the parser. eg arguments.fitname = "junk.fits" is OK, but
 * strcpy(arguments.fitsname,"junk.fits") will cause a segfault.
 */
struct arguments {
      char *args[2];            /* exptime and gain */
      int vlevel;               /* The int (012)associated with the  -v flag */
      char *fitsname;           /* Argument for -o */
      int fitsflg;              /* flag requesting fitsfile, set by -f anything */
...
int main (int argc, char **argv)
{
        ...
        if (_OPENCV == 0 && Dfac != 0){
            printf(
            "\nThe OpenCV libraries are not available, so I cannot display\n\n");
        }
	...
	if(CamNum < 0 || CamNum > numDevices - 1){    /* no Camera */
    ...
    u_short *buf, char *fitsname )
{
    ...
    newcard(fh,"CAMNUM",fh_Int, itoa(camnum),"ASI camera number (0-3)");
