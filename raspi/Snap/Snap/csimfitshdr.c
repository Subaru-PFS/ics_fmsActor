...
newcard(fhead *hdr, char *key, int type, char *valp, char *comment)
{
    int card = hdr->cno;
    int klen ;
    char *cp = (char *)(hdr->header + 80*card);
    int nk ;
    char num[80];
...
timecard(fhead *hdr)
{
    newcard(hdr, (char *)"DATE", fh_Str, fitstimestr(), (char *)"UT");
}

void
endcard(fhead *hdr)
{
    newcard(hdr, (char *)"END", fh_End, 0, 0);
}
...
int main()
{
    ...
    newcard(fh,(char *)"SIMPLE",fh_Log,(char *)"T",(char *)"Natch");
    pcard(fh->cno - 1,(char *)fh->header);    

    newcard(fh,(char *)"BITPIX",fh_Int,itoa(16), (char *)0);
    pcard(fh->cno - 1,(char *)(fh->header+80*((fh->cno) - 1)));    

    newcard(fh,(char *)"NAXIS",fh_Int,itoa(2), (char *)"Its an IMAGE, NO????");
    pcard(fh->cno - 1,(char *)(fh->header+80*((fh->cno) - 1)));    

    newcard(fh,(char *)"NAXIS1",fh_Int, (char *)"4656", (char *)0);
    pcard(fh->cno - 1,(char *)(fh->header+80*((fh->cno) - 1)));    

    newcard(fh,(char *)"NAXIS2",fh_Int,itoa(3520),(char *)"The Ysize deserves a comment which is almost certainly too long for this card");
    pcard(fh->cno - 1,(char *)(fh->header+80*((fh->cno) - 1)));    

    printf("\n  :%80s",gauge);

    newcard(fh,(char *)"FLOATPARAM",fh_Flt,ftoa(123456.7890),(char *)"More Digits?");
    pcard(fh->cno - 1,(char *)(fh->header+80*((fh->cno) - 1)));    

    newcard(fh,(char *)"COMMENT",fh_Cmt,(char *)"All of this is shit", 0);
    pcard(fh->cno - 1,(char *)(fh->header+80*((fh->cno) - 1)));    

    newcard(fh,(char *)"STRING",fh_Str,(char *)"ObservatoryInJungleInBrazil", (char *)"No Comment LOL");
    pcard(fh->cno - 1,(char *)(fh->header+80*((fh->cno) - 1)));    

    newcard(fh,(char *)"SHORTSTRING",fh_Str,(char *)"Obser", (char *)"Short Name, No??");
    pcard(fh->cno - 1,(char *)(fh->header+80*((fh->cno) - 1)));    
