...
/***************************************************************************
Descriptions:
you need to close the camera to free all the resources
...
/***************************************************************************
Description:
Get number of controls available for this camera. the camera need be opened 
at first.

Paras:	
int CameraID: this is get from the camera property use the API 
ASIGetCameraProperty int * piNumberOfControls: pointer to an int to save the 
    number of controls
...
***************************************************************************/
ASICAMERA_API ASI_ERROR_CODE 
ASIGetNumOfControls(int iCameraID, int * piNumberOfControls);
...
/***************************************************************************
Description:
Get controls property available for this camera. the camera need be opened at 
first. user need to malloc and maintain the buffer.

Paras:
int CameraID: this is get from the camera property use the API 
    ASIGetCameraProperty
int iControlIndex: index of control, NOT control type
ASI_CONTROL_CAPS * pControlCaps: Pointer to structure containing the property 
    of the control
    
user needs to malloc the buffer

return:
ASI_SUCCESS : Operation is successful
ASI_ERROR_CAMERA_CLOSED : camera didn't open
ASI_ERROR_INVALID_ID  :no camera of this ID is connected or ID value is out 
    of boundary
***************************************************************************/
ASICAMERA_API ASI_ERROR_CODE 
ASIGetControlCaps(int iCameraID, int iControlIndex, ASI_CONTROL_CAPS * pControlCaps);
...
note:the value of the temperature is the float value * 10 to convert it to 
    long type, control name is "Temperature" because long is the only type 
    for control(except cooler's target temperature, because it is an integer)

Paras:
int CameraID: this is get from the camera property use the 
    API ASIGetCameraProperty
int ControlType: this is get from control property use the 
    API ASIGetControlCaps
...
***************************************************************************/
ASICAMERA_API ASI_ERROR_CODE 
ASIGetControlValue(int iCameraID, ASI_CONTROL_TYPE ControlType, long *plValue, ASI_BOOL *pbAuto);



/***************************************************************************
Description:
Set controls property value and auto value. It will return success and set 
the max value or min value if the value is beyond the boundary
...
ASI_SUCCESS : Operation is successful
ASI_ERROR_CAMERA_CLOSED : camera didn't open
ASI_ERROR_INVALID_ID  :no camera of this ID is connected or ID value is out of 
    boundary
ASI_ERROR_INVALID_CONTROL_TYPE : invalid Control type
ASI_ERROR_GENERAL_ERROR : general error, eg: value is out of valid range; 
    operate to camera hareware failed
***************************************************************************/
ASICAMERA_API ASI_ERROR_CODE 
ASISetControlValue(int iCameraID, ASI_CONTROL_TYPE ControlType, long lValue, ASI_BOOL bAuto);
...
int iHeight,  the height of the ROI area. Make sure iHeight%2 = 0, 
    further, for USB2.0 camera ASI120, please make sure that 
    iWidth*iHeight%1024=0. 
...
ASI_ERROR_INVALID_ID  :no camera of this ID is connected or ID value is out 
    of boundary
ASI_ERROR_INVALID_SIZE, //wrong video format size
ASI_ERROR_INVALID_IMGTYPE, //unsupported image format, make sure iWidth and 
    iHeight and binning is set correct
***************************************************************************/
ASICAMERA_API  ASI_ERROR_CODE 
ASISetROIFormat(int iCameraID, int iWidth, int iHeight,  int iBin, ASI_IMG_TYPE Img_type); 
...
***************************************************************************/
ASICAMERA_API  ASI_ERROR_CODE 
  ASISetStartPos(int iCameraID, int iStartX, int iStartY); 
...
***************************************************************************/
ASICAMERA_API  ASI_ERROR_CODE 
ASIGetStartPos(int iCameraID, int *piStartX, int *piStartY); 
...
***************************************************************************/
ASICAMERA_API  ASI_ERROR_CODE 
ASIGetDroppedFrames(int iCameraID,int *piDropFrames); 
...
***************************************************************************/
ASICAMERA_API ASI_ERROR_CODE 
ASIDisableDarkSubtract(int iCameraID);
...
ASI_ERROR_INVALID_ID  :no camera of this ID is connected or ID value is out of
     boundary
ASI_ERROR_EXPOSURE_IN_PROGRESS: snap mode is working, you need to stop snap 
    first
***************************************************************************/
ASICAMERA_API ASI_ERROR_CODE 
ASIStartVideoCapture(int iCameraID);
...
***************************************************************************/
ASICAMERA_API  ASI_ERROR_CODE 
ASIStopVideoCapture(int iCameraID);
...
int CameraID: this is get from the camera property use the API 
    ASIGetCameraProperty
unsigned char* pBuffer, caller need to malloc the buffer, make sure the size 
    is big enough
		the size in byte:
...
int iWaitms, this API will block and wait iWaitms to get one image. unit is ms
...
***************************************************************************/
ASICAMERA_API ASI_ERROR_CODE 
ASIPulseGuideOn(int iCameraID, ASI_GUIDE_DIRECTION direction);
...
***************************************************************************/
ASICAMERA_API ASI_ERROR_CODE 
ASIPulseGuideOff(int iCameraID, ASI_GUIDE_DIRECTION direction);
...
/***************************************************************************
Description:
Start camera exposure. the following 4 API is usually used when long exposure 
required start exposure  and check the exposure status then get the data
...
ASI_ERROR_INVALID_ID  :no camera of this ID is connected or ID value is out of 
    boundary
ASI_ERROR_VIDEO_MODE_ACTIVE: video mode is working, you need to stop video 
    capture first
***************************************************************************/
ASICAMERA_API ASI_ERROR_CODE  
...
***************************************************************************/
ASICAMERA_API ASI_ERROR_CODE  
ASIGetExpStatus(int iCameraID, ASI_EXPOSURE_STATUS *pExpStatus);
...
int CameraID: this is get from the camera property use the API 
    ASIGetCameraProperty
unsigned char* pBuffer, caller need to malloc the buffer, make sure the size is 
    big enough
the size in byte:
8bit mono:width*height
16bit mono:width*height*2
...
***************************************************************************/
ASICAMERA_API  ASI_ERROR_CODE 
ASIGetDataAfterExp(int iCameraID, unsigned char* pBuffer, long lBuffSize);
...
***************************************************************************/
ASICAMERA_API  ASI_ERROR_CODE 
ASISetID(int iCameraID, ASI_ID ID);
...
***************************************************************************/
ASICAMERA_API ASI_ERROR_CODE 
ASIGetGainOffset(int iCameraID, int *Offset_HighestDR, int *Offset_UnityGain, int *Gain_LowestRN, int *Offset_LowestRN);
